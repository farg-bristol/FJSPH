	...
	if (svar.Bcase == 3 && pi.xi[1] > 2*svar.Pstep)
	{
		switch(cvar.acase)
		{
			...
			case 6: { /*Surface particles, with correction based on surface normal*/
				/*Check if it is a surface particle using force value*/
				// cout << size << endl;
				if (/*SurfC.norm()*pow(svar.Pstep,0.858)*1197/(pi.m) > 0.5*/ size < 165)
				{
					StateVecD Vdiff = cvar.vInf-pi.V;
					Fd = AeroForce(Vdiff, svar, fvar, cvar);
					...
					if(size > 50)
					{
						...
						if (theta <= 1.0 )
						{
							correc = cvar.a*W2Kernel(2*theta,cvar.h1,1)+
								cvar.b*W2Kernel(2*theta,cvar.h2,1);
						}
					}
					Fd = correc*Fd;
				}
			...
			case 7: { /*Vortex Lattice Method*/
				/*Check if it is a surface particle using force value*/
				if (/*SurfC.norm()*pow(svar.Pstep,0.858)*1197/(pi.m) > 0.2*/ size < 150)
				{
					...
					if (theta <= 1.0 )
					{
						correc = cvar.Acorrect*cvar.a*W2Kernel(2*theta,cvar.h1,1)+cvar.b*W2Kernel(2*theta,cvar.h2,1);
					}
