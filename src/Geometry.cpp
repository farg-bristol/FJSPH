...
bool MollerTrumbore(StateVecD const& v0, StateVecD const& v1, StateVecD const& v2, 
                StateVecD const& rayOrigin, StateVecD const& rayVector, real& dt, real& denom)
{
                           ...
                           vector<size_t> const& face, int const& cellID, real& dt, real& denom)
{
    ...
    if(face.size() == 3)
    {   // Just need to do the algorithm, and not worry about whether it does intersect
        MollerTrumbore(cells.verts[face[0]],cells.verts[face[1]],cells.verts[face[2]],rayOrigin, rayVector,dt,denom);
    }   
    else
    {   // Need to check which side of the face is intersected, so need to know the result.
        if(!MollerTrumbore(cells.verts[face[0]],cells.verts[face[1]],cells.verts[face[2]],rayOrigin, rayVector,dt,denom))
            MollerTrumbore(cells.verts[face[0]],cells.verts[face[3]],cells.verts[face[2]],rayOrigin, rayVector,dt,denom);
    }
}
