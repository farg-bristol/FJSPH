...
{ /*Find velocity for a particle at its position*/
    ...
    //     StateVecD y = gamma(ii)*FindInfluenceConditioned(panelData[ii].A,panelData[ii].B,pos) - c; //
    //     c is zero the first time around. StateVecD t = vel + y;              // Alas, sum is big, y
    //     small, so low-order digits of y are lost. c = (t - vel) - y;            // (t - sum) cancels
    //     the high-order part of y; subtracting y recovers negative (low part of y) vel = t; //
    ...
    // Need to introduce a limit to the velocity returned, as possible to be too close to a trailing
    // vortex
...
void VLM::MakeMatrix(void)
{
    ...
    real yend = coords[0]; // *cos(sweep); /* End is now actually the span, and hypotenuse adjusted for
                           // the sweep */
    ...
    { /*i = count in z, (spanwise)*/
        ...
        { /*j = count in x and y, (chordwise)*/
            ...
            /*3/4 control point*/
            StateVecD C(
                x0 + xhalf + dxhalf * (0.75 + real(j)), yhalf, z0 + zhalf + dzhalf * (0.75 + real(j))
            );
        ...
        if (doflap == 1)
        {
            ...
            for (int j = 0; j < flap(2); ++j)
            {
                ...
                /*1/4 chord points*/
                StateVecD A(
                    xflap1 + dxflap1 * (0.25 + real(j)), y1, zflap1 + dzflap1 * (0.25 + real(j))
                );
                StateVecD B(
                    xflap2 + dxflap2 * (0.25 + real(j)), y2, zflap2 + dzflap2 * (0.25 + real(j))
                );
                /*3/4 control point*/
                StateVecD C(
                    xflaphalf + dxflaphalf * (0.75 + real(j)), yhalf,
                    zflaphalf + dzflaphalf * (0.75 + real(j))
                );
                panelData.push_back(Panel(A, B, C, p1, p2, p3, p4));
            }
        }
...
StateVecD VLM::FindInfluenceConditioned(StateVecD const& A, StateVecD const& B, StateVecD const& P) const
{
    ...
    if ((r1.normalized().cross(r2.normalized())).norm() > cutoff)
        coefAB = ipi4 * (numer / denom) * (r0.dot(r1.normalized() - r2.normalized())); // Omega term
    else
    ...
    // Cut off if denomenator is too small
    if ((r1.normalized().cross(r2.normalized())).norm() > cutoff)
        coefA = ipi4 * r0.norm() * (numer / denom) * (1 - r0.dot(r2) / (r0.norm() * r2.norm()));
    else
        return StateVecD::Zero();
    ...
    // Cut off if denomenator is too small
    if ((r1.normalized().cross(r2.normalized())).norm() > cutoff)
        coefB = ipi4 * r0.norm() * (numer / denom) * (r0.dot(r1) / (r1.norm() * r0.norm()) + 1.0);
    else
        return StateVecD::Zero();
