...
const StateVecD VLM::getVelocity(const StateVecD pos)
{	/*Find velocity for a particle at its position*/
    StateVecD vel = StateVecD::Zero();

    for(int i = 0; i<npanels; ++i)
        vel += gamma(i)*VLM::FindInfluence(panelData[i].A,panelData[i].B,pos);
    
    return vel+Freestream;
}

void VLM::write_VLM_Panels(std::string &folder)
{	
...
std::string VLM::getS(std::ifstream& In)
{
    std::string line;
    getline(In,line);
    return line; 
...
Eigen::Vector2i VLM::getIVec(std::ifstream& In)
{
    std::string line;
    getline(In,line);
...
Eigen::Matrix<real,2,1> VLM::getvec(std::ifstream& In)
{
    std::string line;
    getline(In,line);
    std::istringstream sline(line);
...
Eigen::Vector3i VLM::get3dVector(std::ifstream& In)
{
    std::string line;
    getline(In,line);
    std::istringstream sline(line);
...
const StateVecD VLM::FindInfluence(StateVecD const& A, StateVecD const& B, StateVecD const& C)
{	
