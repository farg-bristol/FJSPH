...
void Place_Point(
    FLUID const& fvar, StateVecD const& xi, StateVecD const& v, real const& rho, real const& press,
    size_t const& pState, size_t& pID, SIM& svar, SPHState& pn
)
{
...
inline void Droplet_GisslerForce(
    AERO const& avar, StateVecD const& Vdiff, StateVecD const& norm, real const& rho, real const& press,
    real const& mass, real const& lam, real const& woccl, StateVecD& totAcc, StateVecD& sphereAcc,
    StateVecD& plateAcc
)
{
    ...
    sphereAcc = (1.0 - woccl) * frac1 * q * avar.gamma * press * Cdl * Adrop / mass;

    plateAcc = (1.0 - woccl) * frac2 * q * avar.gamma * press * avar.aPlate /
               mass; /* Cd of plate assumed to be 1 */
}

inline void Droplet_InducedPressure(
    AERO const& avar, StateVecD const& Vdiff, StateVecD const& norm, real const& Pbasei, real const& lam,
    SPHPart const& pi, StateVecD& totAcc, StateVecD& sphereAcc, StateVecD& plateAcc
)
{
    ...
    // #pragma omp critical
    // cout << Cp_s << "  " << Cp_p << "  " << Cp_tot << "  " << theta << "  " << pi.curve << "  " << Pi
    // <<  endl;
    ...
    StateVecD const acc_drop =
        0.5 * Vdiff * Vdiff.norm() * avar.isossqr * avar.gamma * pi.cellP * avar.aSphere * Cdi / pi.m;
...
void Droplet_Drag_Sweep(SIM& svar, FLUID& fvar, AERO& avar)
{
    ...
    if (svar.Reynolds.empty())
    {
        ...
        {
            cout << "ERROR: No Reynolds or velocities have been defined for the sweep. Stopping. "
                 << endl;
            exit(-1);
        }
        else
    ...
    for (size_t res_i = 0; res_i < svar.nacross.size(); ++res_i)
    {
        ...
        cout << endl
             << "Original resolution: " << res << " Actual resolution: " << new_res[res_i]
             << "  Particle count: " << svar.simPts << endl;
...
#pragma omp ordered
        ...
        for (size_t Re_i = 0; Re_i < svar.Reynolds.size(); ++Re_i)
        {
            ...
            for (size_t ii = 0; ii < to_test.size(); ++ii)
            {
                ...
                /* Original Gissler */
                Droplet_GisslerForce(
                    avar, Vdiff, norm[ii], avar.rhog, avar.pRef, pi.m, lam[ii], pi.woccl, totalAcc_g,
                    sphereAcc_g, plateAcc_g
                );

                /* Induced pressure based model */
                Droplet_InducedPressure(
                    avar, Vdiff, norm[ii], 0.0, lam[ii], pi, totalAcc_ip, sphereAcc_ip, plateAcc_ip
                );
...
// real dynp = 1.0;
#if SIMDIM == 3
            real dynp = 0.5 * (vel * vel / (avar.sos * avar.sos)) * avar.gamma * avar.pRef * 0.25 *
                        M_PI * svar.diam * svar.diam;
#else
            ...
            cout << "    Gissler: " << drop_data[res_i][Re_i][0]
                 << "  Induced Pressure: " << drop_data[res_i][Re_i][3] << endl;
        }
    ...
    fout << "VARIABLES = \"diameter resolution\", \"Gissler normalised drag\","
         ...
         << "\"Induced pressure total drag\", \"Induced pressure sphere drag \", \"Induced pressure "
            "plate drag\""
         << endl;
    ...
    for (size_t ii = 0; ii < svar.Reynolds.size(); ++ii)
    {
        ...
        for (size_t jj = 0; jj < svar.nacross.size(); ++jj)
        {
            ...
            fout << new_res[jj] << " " << drag_g << " " << drop_data[jj][ii][0] << " "
                 << drop_data[jj][ii][1] << " " << drop_data[jj][ii][2] << " " << drag_ip << " "
                 << drop_data[jj][ii][3] << " " << drop_data[jj][ii][4] << " " << drop_data[jj][ii][5]
                 << endl;
        }
        ...
        for (size_t jj = 0; jj < svar.nacross.size(); ++jj)
        {
            fout << new_res[jj] << " 1 " << big_cd << " " << big_cd << " " << big_cd << " 1 " << big_cd
                 << " " << big_cd << " " << big_cd << endl;
    ...
    for (size_t jj = 0; jj < svar.nacross.size(); ++jj)
    {
        ...
        for (size_t ii = 0; ii < svar.velocities.size(); ++ii)
        {
            ...
            fout2 << Re << " " << drag_g << " " << drop_data[jj][ii][0] << " " << drop_data[jj][ii][1]
                  << " " << drop_data[jj][ii][2] << " " << drag_ip << " " << drop_data[jj][ii][3] << " "
                  << drop_data[jj][ii][4] << " " << drop_data[jj][ii][5] << endl;
        }
    }
