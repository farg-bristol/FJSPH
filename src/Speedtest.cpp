...
#pragma omp parallel for
    ...
    cells.alloc(ni * nj, (ni - 1) * (nj - 1), 2 * ni * nj - ni - nj, 0);
...
{
    sph_group()
        : pn(SPHState(1, SPHPart(StateVecD::Zero(), StateVecD::Zero(), 0.0, 0.0, 0.0, 0, 0))),
...
void Speed_Test_Sweep(SIM& svar, FLUID& fvar, AERO& avar)
{
    ...
    for (int const res : svar.nacross)
    {
        ...
        cout << "\nIPT total time: " << time_ipt << "  nSuccess: " << svar.nSuccess
             << "  nFailed: " << svar.nFailed << endl;
