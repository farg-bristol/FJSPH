...
namespace PoissonSample
{
    class PRNG
    {
      public:
        ...
        real randomReal() { return static_cast<real>(dis_(gen_)); }
        ...
        bool isInNeighbourhood(StateVecD const& point)
        {
            ...
            for (int ii = g(0) - D; ii < g(0) + D; ii++)
            {
                for (int jj = g.y() - D; jj < g.y() + D; jj++)
                {
                    ...
                    {
                        if (ii >= 0 && ii < int(w_) && jj >= 0 && jj < int(h_) && kk >= 0 &&
                            kk < int(d_))
                        {
...
// the new point is generated around the point (x, y)
...
#if SIMDIM == 3
        return StateVecD(
            p(0) + radius * cos(angle1) * sin(angle2), p(1) + radius * sin(angle1) * sin(angle2),
    ...
    **/
    SPHState generatePoissonPoints(
        SIM& svar, FLUID const& fvar, AERO const& avar, MESH const& cells, uint const& host,
        SPHState const& pnp1, OUTL const& outlist /* , StateVecD const& norm, StateVecD const& avgV */
    )
    {
...
#endif
#if SIMDIM == 3
        const StateVecD origin(
            pnp1[host].xi(0) - 2 * fvar.H, pnp1[host].xi(1) - 2 * fvar.H, pnp1[host].xi(2) - 2 * fvar.H
        );
#endif
...
/*Try and add a point where it won't conflict*/
        ...
        do
        { /*Generate a random number between 0 and 1, then normalise it to the grid.*/
#if SIMDIM == 2
            firstPoint =
                StateVecD(generator.randomReal() * 4 * fvar.H, generator.randomReal() * 4 * fvar.H);
#endif
#if SIMDIM == 3
            firstPoint = StateVecD(
                generator.randomReal() * 4 * fvar.H, generator.randomReal() * 4 * fvar.H,
            ...
            for (uint ii = 0; ii < sampleLimit; ii++)
            {
                ...
                if (canFitPoint == true && grid.isInNeighbourhood(newPoint) == false)
                {
                    ...
                    samplePoints.push_back(
                        SPHPart(newPoint + origin, StateVecD::Zero(), 0.0, 0.0, 0.0, GHOST, 0)
                    );
...
void PoissonGhost(
    SIM& svar, FLUID const& fvar, AERO const& avar, MESH const& cells, Sim_Tree& NP1_INDEX,
    ...
    for (size_t ii = start; ii < end; ii++)
    {
        if (pnp1[ii].surf == 1)
        {
            ...
            SPHState ghost_particles =
                PoissonSample::generatePoissonPoints(svar, fvar, avar, cells, ii, pnp1, outlist);
...
void check_if_too_close(
    Sim_Tree const& NP1_INDEX, real const& sr, SPHState const& pnp1, StateVecD const& xi,
    StateVecD const& vel, real const& dens, real const& mass, real const& press, size_t const& pID,
    SPHState& ghost_particles, size_t& nGhost
)
{
...
void LatticeGhost(
    SIM& svar, FLUID const& fvar, AERO const& avar, MESH const& cells, Sim_Tree& SPH_TREE, OUTL& outlist,
    SPHState& pn, SPHState& pnp1, LIMITS const& limits
)
{
    // size_t const& start = svar.bndPts;
        ...
        for (size_t ii = limits[block].index.first; ii < limits[block].index.second; ii++)
        {
            if (pnp1[ii].surf == 1)
            {
                ...
                std::vector<neighbour_index> matches =
                    radius_search(SPH_TREE, pnp1[ii].xi, search_radius);
                ...
                for (real x = 0.0; x < (2 * fvar.H + svar.dx); x += svar.dx)
                {
                    for (real y = 0.0; y < (2 * fvar.H + svar.dx); y += svar.dx)
                    {
                        ...
                        for (real z = 0.0; z < (2 * fvar.H + svar.dx); z += svar.dx)
                        {
                            ...
                            check_if_too_close(
                                SPH_TREE, sr, pnp1, xi, vel, rho, mass, press, pID, ghost_particles,
                                nGhost
                            );
                        ...
                        check_if_too_close(
                            SPH_TREE, sr, pnp1, xi, vel, rho, mass, press, pID, ghost_particles, nGhost
                        );
...
void Check_If_Ghost_Needs_Removing(
    SIM& svar, FLUID const& fvar, Sim_Tree& NP1_INDEX, SPHState& pn, SPHState& pnp1
)
{
