...
inline StateMatD GetRotationMat(StateVecD& angles)
{
    ...
    StateMatD m(
        Eigen::AngleAxis<real>(angles[0], StateVecD::UnitX()) *
...
#else
    StateMatD rot;
    rot << cos(angles(0)), -sin(angles(0)), sin(angles(0)), cos(angles(0));
...
inline std::pair<StateVecD, StateVecD> Find_MinMax(SIM& svar, const SPHState& pnp1)
{
    ...
    auto xC = std::minmax_element(
        pnp1.begin(), pnp1.end(), [](SPHPart p1, SPHPart p2) { return p1.xi(0) < p2.xi(0); }
    );
    auto yC = std::minmax_element(
        pnp1.begin(), pnp1.end(), [](SPHPart p1, SPHPart p2) { return p1.xi(1) < p2.xi(1); }
    );
#if SIMDIM == 3
    auto zC = std::minmax_element(
        pnp1.begin(), pnp1.end(), [](SPHPart p1, SPHPart p2) { return p1.xi(2) < p2.xi(2); }
    );
...
inline int LessThanREError(StateP1MatD const& A)
{
    ...
    a1 = fabs(A(0, 2) - A(3, 2)) * (fabs((A(1, 0) - A(3, 0)) * (A(2, 1) - A(3, 1))) +
                                    fabs((A(1, 1) - A(3, 1)) * (A(2, 0) - A(3, 0))));
    a2 = fabs(A(1, 2) - A(3, 2)) * (fabs((A(2, 0) - A(3, 0)) * (A(0, 1) - A(3, 1))) +
                                    fabs((A(2, 1) - A(3, 1)) * (A(0, 0) - A(3, 0))));
    a3 = fabs(A(2, 2) - A(3, 2)) * (fabs((A(0, 0) - A(3, 0)) * (A(1, 1) - A(3, 1))) +
                                    fabs((A(0, 1) - A(3, 1)) * (A(1, 0) - A(3, 0))));

    if (fabs(A.determinant()) <= MERROR * (a1 + a2 + a3))
    {
        // #pragma omp critical
        // {
        // cout << "Volume is inside the tolerance of round-off error. Need to do some form of
        // tiebreaking." << endl; cout << "Matrix: " << endl << A << endl; cout << "MERROR: " << MERROR
        // << endl; cout << "Components: " << a1 << "  " << a2 << "  " << a3 << endl; cout <<
        // A.determinant() <<  " <= " <<  MERROR*(a1+a2+a3) << endl;
        // }
        return 1;
    }
