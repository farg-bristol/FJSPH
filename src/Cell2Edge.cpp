...
{
    /*Standard contructor*/
    FACE(uint const nElem, uint const nPts) : nPnts(nPts), nElem(nElem) { verts.reserve(nPts); }
...
template <typename T> void Get_Number(string const& line, string const& param, T& value)
{
...
void Get_Data(
    int& fin, size_t const& nPnts, FACE& fdata, vector<int> markers, int const& symPlane1,
    int const& symPlane2
)
{
#ifdef DEBUG
    ...
    else
    {
        ...
        vector<vector<size_t>> localVec =
            Get_Element(fin, "points_of_surfacequadrilaterals", nSQua, nPpSQ);
    ...
    if (faceVec.size() != nMarkers)
    {
        ...
        cout << "Number of surface markers: " << nMarkers << "  Number in vector: " << faceVec.size()
             << endl;
    }
...
void Add_Edge(std::pair<uint, uint> const& edge, std::pair<int, int> const& leftright, EDGE& edata)
{
...
#pragma omp atomic
            ...
            // #ifdef DEBUG
            // 				#pragma omp critical
            // 				cout << "Thread " << omp_get_thread_num() << " processed cells: "
            // << localCount << " (" << 				100.0 * float(localCount)/(float(cdata.numElem))  << "%)" <<
            // endl; #endif
            // 		    }
            // 		    else
            // 		    {
            ...
            // 						100.0 * float(cellSum)/(float(cdata.numElem))  <<
            // "%)" <<  endl;
...
#pragma omp parallel shared(newVerts, edata, vertInUse)
    {
        ...
        // 			cout << "Point: " << edge.first << "  index1: " << ifirst << "  lower: "
        // << idx << endl;
        // 		}
        // 		// auto lower2 = std::lower_bound(vertInUse.begin(), vertInUse.end(),
        // edge.second);
...
void Write_Edge_Data(const string& meshIn, const EDGE& edata)
{
    ...
    if ((retval = nc_put_att_text(
             meshID, NC_GLOBAL, "converted_mesh_filename", meshIn.length(), meshIn.c_str()
         )))
    {
    ...
    if ((retval = nc_def_var(meshID, "points_of_element_edges", NC_INT, 2, dimIDs, &edgeVarID)))
    {
    ...
    if ((retval = nc_def_var(meshID, "left_element_of_edges", NC_INT, 1, &nEdgeID, &leftVarID)))
    {
    ...
    if ((retval = nc_def_var(meshID, "right_element_of_edges", NC_INT, 1, &nEdgeID, &rightVarID)))
    {
    ...
    if ((retval = nc_def_var(meshID, "boundarymarker_of_surfaces", NC_INT, 1, &nMarkID, &markID)))
    {
    ...
    if ((retval = nc_def_var(meshID, "vertices_in_use", NC_INT, 1, &nPntsID, &usedPID)))
    {
...
)
{
    ...
    if (retval)
    {
        cout << "Failed to write \"" << varName << "\". frame: " << outputZone
             << ". varCount: " << varCount << endl;
        exit(-1);
    }
    varCount++;
}
...
void Write_Edge_Tecplot_Binary(const EDGE& edata)
{
    ...
    if (TECINI142(
            title.c_str(), varNames.c_str(), file.c_str(), ".", &fileFormat, &fileType, &debug,
            &fileIsDouble
        ))
    {
    ...
    if (TECPOLYZNE142(
            "Polygonal Quad Zone", &zoneType, &numNodes, &numElems, &numFaces, &numFaceNodes, &solTime,
...
#pragma omp parallel for
    ...
    if (TECPOLYFACE142(&faceOffset, NULL, &faceNodes[0], &left[0], &right[0]))
    {
...
void Write_Edge_szplt(const EDGE& edata)
{
    ...
    if (tecFileWriterOpen(
            file.c_str(), title.c_str(), varNames.c_str(), fileType, fileFormat, 1, NULL, &fileHandle
        ))
    {
        cout << "Failed to open " << file << endl;
        exit(-1);
    ...
    if (tecZoneCreatePoly(
            fileHandle, group.c_str(), zoneType, edata.nPnts, edata.nEdge, edata.nElem,
            totalNumFaceNodes, &varTypes[0], &shareVarFromZone[0], &valueLocation[0], &passiveVarList[0],
            0, 0, 0, &outputZone
        ))
    {
...
#pragma omp parallel for
    ...
    tecZoneWritePolyFaces32(
        fileHandle, outputZone, 0, edata.nEdge, &faceCounts[0], &faceNodes[0], &left[0], &right[0], 1
    );
...
void Write_griduns(const EDGE& edata)
{
    ...
    fout << std::setw(w) << edata.nElem << std::setw(w) << edata.nEdge << std::setw(w) << edata.nPnts
         << endl;
    ...
    if (leftright == 1)
    {
        ...
        { /*Add 1 (for fortran bull) if not a boundary edge*/
            ...
            {
                fout << std::setw(w) << edata.edges[ii].second + 1 << std::setw(w)
                     << edata.edges[ii].first + 1;
                fout << std::setw(w) << edata.celllr[ii].second << std::setw(w)
                     << edata.celllr[ii].first + 1;
            }
            else if (edata.celllr[ii].second == -2)
            {
                fout << std::setw(w) << edata.edges[ii].first + 1 << std::setw(w)
                     << edata.edges[ii].second + 1;
                fout << std::setw(w) << edata.celllr[ii].first + 1 << std::setw(w)
                     << edata.celllr[ii].second;
            }
            else
            {
                fout << std::setw(w) << edata.edges[ii].first + 1 << std::setw(w)
    ...
    else
    {
        ...
        { /*Add 1 (for fortran bull) if not a boundary edge*/
            ...
            else
            {
                fout << std::setw(w) << edata.edges[ii].first + 1 << std::setw(w)
                     << edata.edges[ii].second + 1;
                fout << std::setw(w) << edata.celllr[ii].second + 1 << std::setw(w)
                     << edata.celllr[ii].first + 1;
            }
    ...
    {
        fout << std::setw(8) << ii + 1 << std::setw(w) << edata.verts[ii][0] << std::setw(w)
             << edata.verts[ii][1] << endl;
    }
