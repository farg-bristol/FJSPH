...
namespace HDF5
{
    ...
    void Read_Vector_Attribute(hid_t const& fin, std::string const& attr_name, StateVecD& attr_data)
    {
        ...
        if(SIMDIM > dims[0])
        {
            printf("ERROR: Vector dimension is smaller than simulation dimension. Will cause a fault so stopping.\n");
            exit(-1);
        }
...
void Write_Zone_Attributes(hid_t const& zone, bound_block const& limits)
{
	...
	if(limits.nTimes != 0)
	{
		...
		for(size_t time = 0; time < limits.nTimes; time++)
		{
			...
			HDF5::Write_Real_Attribute(zone, "Velocity " + std::to_string(time) + " x",limits.vels[time][0]);
			HDF5::Write_Real_Attribute(zone, "Velocity " + std::to_string(time) + " y",limits.vels[time][1]);
			#if SIMDIM == 3
			HDF5::Write_Real_Attribute(zone, "Velocity " + std::to_string(time) + " z",limits.vels[time][2]);
			#endif
		}
	}
	else
 ...
 * @return void
 */
void Write_HDF5(SIM& svar, FLUID const& fvar, AERO const& avar, SPHState const& pnp1, LIMITS const& limits)
{
...
//  * @return void
//  */
// void Write_h5part(std::string const& prefix, SIM const& svar, Particles const& parts, GasParticle const& gas, 
