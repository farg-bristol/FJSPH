...
void check_input(shape_block& block, real& globalspacing, int& fault)
{
    ...
    {
        printf(
            "ERROR: Cylinder block \"%s\" subtype not defined appropriately. Choose from the following "
            "options:\n",
            block.name.c_str()
        );
        printf("\t1. Hollow\n\t2. Solid\n");
    ...
    //     if(block.ni < 0)
    //     {
    //         printf("WARNING: Cylinder block \"%s\" globalspacing has not been defined. Using global
    //         globalspacing.\n",block.name.c_str()); block.dx = globalspacing;
    //     }
    //     else
    //     {
