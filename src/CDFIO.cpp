...
void Average_Point_Data_to_Cell(SIM const& svar, vector<StateVecD> const& vel, 
...
namespace TAU
{
	...
	void Place_Edges(int &fin, size_t const &nElem, size_t const &nPnts, size_t const &nEdge, MESH &cells)
	{
		...
		#pragma omp parallel default(shared)/* shared(leftright) */ reduction(max:maxedge) reduction(min:minedge)
		{
			vector<size_t> local;
			#pragma omp for schedule(static) nowait
			for (size_t ii = 0; ii < nEdge; ++ii)
		...
		#pragma omp parallel shared(leftright) reduction(max:maxedge) reduction(min:minedge)
		{
			...
			for (size_t ii = 0; ii < nFace; ++ii)
			{
				...
				if(cells.faces[ii].size() == 3)
				{
					...
					min_e = std::min((cells.verts[face[0]] - cells.verts[face[1]]).norm(),
							std::min((cells.verts[face[0]] - cells.verts[face[2]]).norm(),
									(cells.verts[face[1]] - cells.verts[face[2]]).norm()));
				}
				else
				{
					...
					min_e = std::min((cells.verts[face[0]] - cells.verts[face[1]]).norm(),
							std::min((cells.verts[face[1]] - cells.verts[face[2]]).norm(),
							std::min((cells.verts[face[2]] - cells.verts[face[3]]).norm(),
