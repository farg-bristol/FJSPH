...
struct ProgressBar
{
    ...
    { // Start with an empty bar
        ...
        cout << "\rPercent: [" << spaces << "] " << int(round(percent * 100)) << "%" << std::flush;
    };
...
#pragma omp parallel for
    ...
    size_t surface_quads(int& meshID, std::vector<std::array<int, 7>>& faces)
    {
        ...
        else
        {
            ...
            Get_Dimension(
                meshID, "points_per_surfacequadrilateral", num_points_per_quad_dim, num_points_per_quad
            );
            points_of_quad =
                Get_Element(meshID, "points_of_surfacequadrilaterals", num_quad, num_points_per_quad);
        }
...
#pragma omp parallel for
    ...
    size_t surface_trigs(int& meshID, std::vector<std::array<int, 7>>& faces)
    {
        ...
        else
        {
            ...
            Get_Dimension(
                meshID, "points_per_surfacetriangle", num_points_per_trig_dim, num_points_per_trig
            );
            points_of_trig =
                Get_Element(meshID, "points_of_surfacetriangles", num_trig, num_points_per_trig);
        }
...
#pragma omp parallel for
    ...
    void write_points(int& fin, int& fout, size_t const& nPnts, int& ptsxID, int& ptsyID, int& ptszID)
    {
        // # Write points
    ...
    void Write_Face_Data(
        int& fin, string const& meshIn, string const& meshOut,
        vector<std::pair<string, string>> const& attrdata, std::vector<std::array<int, 7>> const& faces,
        size_t const& nPnts, size_t const& nElem, size_t const& nFace, size_t const& nSurf
    )
    {
#ifdef DEBUG
        ...
        if ((retval = nc_put_att_text(
                 fout, NC_GLOBAL, "converted_mesh_filename", meshIn.length(), meshIn.c_str()
             )))
        {
        ...
        {
            if ((retval = nc_put_att_text(
                     fout, NC_GLOBAL, attrdata[ii].first.c_str(), attrdata[ii].second.length(),
                     attrdata[ii].second.c_str()
                 )))
            {
...
#pragma omp atomic
        ...
        if (ntFaces != 0)
        {
            Define_Variable(fout, "points_of_triangles", NC_INT, 2, dimIDs, &faceTVarID);
        }
        ...
        if (nqFaces != 0)
        {
            dimIDs[0] = nQFaceID;
            dimIDs[1] = nPpQFcID;
            Define_Variable(fout, "points_of_quadrilaterals", NC_INT, 2, dimIDs, &faceQVarID);
        }

        Define_Variable(fout, "left_element_of_faces", NC_INT, 1, &nFaceID, &leftVarID);
        Define_Variable(fout, "right_element_of_faces", NC_INT, 1, &nFaceID, &rightVarID);
        Define_Variable(fout, "boundarymarker_of_surfaces", NC_INT, 1, &nMarkID, &markID);
    ...
    void Write_Tecplot_Binary(
        int& fin, string const& meshOut,
        std::vector<std::pair<std::string, std::string>> const& attrdata,
        std::vector<int> const& markers, std::vector<size_t> const& surfaceCount, int* const facemarkers,
        std::vector<std::array<int, 7>> const& faces, size_t const& nPnts, size_t const& nElem,
        size_t const& nFace
    )
    {
        ...
        if (TECINI142(
                title.c_str(), varNames.c_str(), meshOut_.c_str(), ".", &fileFormat, &fileType, &debug,
                &fileIsDouble
            ))
        {
...
#pragma omp parallel for reduction(+ : ntFaces, nqFaces)
        ...
        if (TECPOLYZNE142(
                "Volume data", &zoneType, &numNodes, &numElems, &numFaces, &numFaceNodes, &solTime,
                &strandID, &unused, &numBoundaryFaces, &numBoundaryConnections, NULL,
                NULL, // All nodal variables
                NULL, &shareConnectivity
            ))
        {
        ...
        if (TECPOLYFACE142(&faceOffset, faceNodeCount, &faceNodes[0], &left[0], &right[0]))
        {
        ...
        for (size_t ii = 0; ii < surfaceCount.size(); ii++)
        {
            ...
            // vector<int32_t> snodes(nodeIndex.begin(),nodeIndex.end()); //recast to a vector to index
            // in
            ...
            // #pragma omp parallel for
            for (std::unordered_set<int32_t>::iterator it = nodeIndex.begin(); it != nodeIndex.end();
                 it++)
            {
...
#pragma omp parallel for
            ...
            if (TECZNE142(
                    zonename.c_str(), &zoneType, &numNodes, &numElems, &nFaces, &iMax, &jMax, &kMax,
                    &solTime, &strandID, &unused, &isBlock, 0, 0, 0, /* TotalNumFaceNodes */
...
int main(int argc, char* argv[])
{
    ...
    // internal face and boundary face lists. Just sort the boundary faces, since they're at the start of
    // the list
            ...
            for (int& nn : node_to_face[n])
            {
                if (nn != -1 && nn < face_count)
                {
                    ...
                    if (f.nodes[3] != -1) // Quad
                    {
                        ...
                        {
                            if (f.nodes[1] == fn.nodes[3] && f.nodes[2] == fn.nodes[2] &&
                                f.nodes[3] == fn.nodes[1])
                            {
                                found = true;
                                f.right = fn.left;
                                f.zone = fn.zone;
            ...
            if (!found)
            {
                ...
                for (int& nn : node_to_face[n])
                {
                    if (nn != -1)
                    {
                        ...
                        fn.from_array(faces[nn]);
                        printf(
                            "%d %d %d %d zone: %d\n", fn.nodes[0], fn.nodes[1], fn.nodes[2], fn.nodes[3],
                            fn.zone
                        );
                    }
                }
    ...
    TAUtoFace::Write_Face_Data(
        meshID, meshIn, meshOut, attrdata, f_list, nPnts, nElem, nFace, nSurfElem
    );
    ...
    cout << "All complete! Total time taken: " << duration_cast<microseconds>(t4 - t1).count() / 1e6
         << endl;
