...
inline real pressure_equation(real const& rho, real const& B, real const& gam, 
							real const& Cs, real const& rho0, real const& backP)
{
	#ifdef COLEEOS
		(void)Cs;
...
inline StateVecD ALEMomentum(SPHPart const& pi, SPHPart const& pj, real const& Vj, StateVecD const& gradK)
{
...
inline real Continuity_dSPH(StateVecD const& Rji, real const& idist2, real const& HSQ, StateVecD const& Grad, 
				real const& volj, SPHPart const& pi, SPHPart const& pj)
{
...
inline StateVecD Linear_ArtVisc(StateVecD const& Rji, StateVecD const& Vji, real const idist2, 
