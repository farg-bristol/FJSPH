				...
				OUTL const& outlist, SPHState& pnp1)
{
	...
	for(size_t ii = start; ii < end; ++ii )
	{
		...
		{	/* Neighbour list loop. */
			...
			#ifndef NODSPH
			if(pj.b > PISTON)
				Rrhod += Continuity_dSPH(Rji,idist2,fvar.HSQ,gradK,volj,pi.gradRho,pj.gradRho,pi,pj);
			else
...
/* DELTAP const& dp, */ SPHState& pnp1)
{
	...
	#pragma omp parallel default(shared)
	{
		...
		{	
			...
			deltaU = std::min(deltaU.norm(), std::min(maxUij/2.0,svar.maxshift)) * deltaU.normalized();
